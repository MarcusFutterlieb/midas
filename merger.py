...
def merger():
    ...
    with open('library/bmw_vz-aktie__backup.csv', 'r') as t1, open('library/bmw_vz-aktie__new.csv', 'r') as t2, open(
            'library/bmw_vz-aktie__merge.csv', 'w') as merge:
            ...
            if (cnt == (3 * formatFactor) - 1):
                ...
                if (postionInNew != dateStartPosition):
                    if (postionInNew == -1):
                        ...
                        print(
                            'Could not find common date in files --> Has it been more than a month since you updated last?');
                        additionalDates = 0;
                    else:
                        # print (postionInNew);
